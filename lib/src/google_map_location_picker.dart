...
class LocationPickerState extends State<LocationPicker> {
          ...
          child: Container(
            ...
            child: Row(
              children: <Widget>[
                ...
                Expanded(
                  child: Text(
                    S.of(context).finding_place != "" ? S.of(context).finding_place : 'Finding place...',
    ...
    LocationUtils.getAppHeaders().then((headers) => http.get(Uri.parse(endpoint), headers: headers)).then((response) {
      if (response.statusCode == 200) {
        ...
        if (predictions.isEmpty) {
          ...
          aci.text = S.of(context).no_result_found != "" ? S.of(context).no_result_found : 'No result found';
          aci.offset = 0;
          aci.length = 0;
  ...
  void decodeAndSelectPlace(String placeId) {
    ...
    final endpoint = "https://maps.googleapis.com/maps/api/place/details/json?key=${widget.apiKey}" +
        "&placeid=$placeId" +
        '&language=${widget.language}&sensor=true';
    ...
    LocationUtils.getAppHeaders().then((headers) => http.get(Uri.parse(endpoint), headers: headers)).then((response) {
      if (response.statusCode == 200) {
        ...
        _locationProvider!.setSelectedPlacedId(placeId);
      ...
      child: Builder(builder: (context) {
        ...
        return Scaffold(
          ...
          body: MapPicker(
            ...
            necessaryField: widget.necessaryField,
          ),
        );
      }),
    );
